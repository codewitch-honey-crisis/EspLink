		...
		public static void PrintUsage(List<CmdSwitch> switches, int width = 0, TextWriter writer = null, string switchPrefix = null)
		{
			...
			else // grouped usage
			{
				...
				for (int i = 0; i < grps.Length; ++i)
				{
					...
					var str = exe + " ";
					writer.Write(str);
					writer.WriteLine(GetUsageArguments(switches, switchPrefix, width, str.Length, true, grp));
					writer.WriteLine();
					writer.Write(GetUsageCommandDescription(switches, switchPrefix, width, grp));
				}
			}
		}
		/// <summary>
