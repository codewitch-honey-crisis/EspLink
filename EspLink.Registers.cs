...
namespace EL
{
	...
	{
		public async Task<uint> ReadRegAsync( uint address, CancellationToken cancellationToken, int timeout = -1)
		{
		...
		public async Task<(uint Value, byte[] Data)> WriteRegAsync(uint address, uint value, CancellationToken cancellationToken, uint mask = 0xFFFFFFFF, uint delayUSec = 0, uint delayAfterUSec = 0, int timeout = -1)
		{
